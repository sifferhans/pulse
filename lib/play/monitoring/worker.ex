defmodule Play.Monitoring.Worker do
  @moduledoc """
  Per-monitor GenServer. Schedules itself, runs an HTTP probe, persists the
  result via `Play.Monitoring.record_check/1`, and updates incident state.
  """
  ...
  alias Play.Monitoring.{Monitor, Probe}
  ...
  def via(monitor_id) when is_integer(monitor_id) do
    {:via, Registry, {Play.Monitoring.WorkerRegistry, monitor_id}}
  end

  def whereis(monitor_id) do
    case Registry.lookup(Play.Monitoring.WorkerRegistry, monitor_id) do
      [{pid, _}] -> pid
      [] -> nil
    end
  end
